...
def run(
    ...
    for case in cases:
        try:
            result = ask(base_url, case["question"], limit=limit)
        ...
        evidence_phrase = case.get("evidence_phrase")
        if evidence_phrase:
            retrieval_ok = any(
                evidence_phrase.lower() in citation["chunk_content"].lower()
    ...
    total = len(cases)
    print()
    print(f"Limit: {limit} chunks per question")
    ...
    if errors:
        print(f"Errors (request failed, counted as failures): {errors}")
...
if __name__ == "__main__":
    ...
    sys.exit(run(args.base_url, args.minimum_accuracy, args.questions, args.limit))